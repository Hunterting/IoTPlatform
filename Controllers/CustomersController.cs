...
public class CustomersController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<CustomerDetailDto>>> GetCustomer(long id)
    {
        try
        {
            ...
            if (customer == null)
            {
                return ApiResponse<CustomerDetailDto>.NotFound("客户不存在");
            }
            ...
            {
                return ApiResponse<CustomerDetailDto>.Forbidden("无权访问该客户");
            }
        ...
        {
            return ApiResponse<CustomerDetailDto>.Error(ex.Message);
        }
    }
    ...
    public async Task<ActionResult<ApiResponse<CustomerDto>>> CreateCustomer([FromBody] CreateCustomerRequest request)
    {
        try
        {
            ...
            if (exists)
            {
                return ApiResponse<CustomerDto>.BadRequest("客户代码或应用代码已存在");
            }
        ...
        {
            return ApiResponse<CustomerDto>.Error(ex.Message);
        }
    }
    ...
    public async Task<ActionResult<ApiResponse<CustomerDto>>> UpdateCustomer(long id, [FromBody] UpdateCustomerRequest request)
    {
        try
        {
            ...
            if (customer == null)
            {
                return ApiResponse<CustomerDto>.NotFound("客户不存在");
            }
            ...
            if (role != Roles.SUPER_ADMIN && customer.AppCode != appCode)
            {
                return ApiResponse<CustomerDto>.Forbidden("无权修改该客户");
            }
    ...
    public async Task<ActionResult<ApiResponse>> DeleteCustomer(long id)
    {
        try
        {
            ...
            {
                return ApiResponse.BadRequest("客户有关联数据，无法删除");
            }
